...
class QwenImage {
    ...
    async getTask(prompt, size, type) {
        ...
        // pastikan data sesuai struktur, kalo ga ada errornya biar keliatan
        if (!resp.data.messages || !resp.data.messages[1] || !resp.data.messages[1].extra || !resp.data.messages[1].extra.wanx || !resp.data.messages[1].extra.wanx.task_id) {
    ...
    async checkTask(id) {
        while (true) {
            ...
            throw new Error(`Task failed with status: ${data.task_status}`);
        }
    }
